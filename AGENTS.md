...
- 2026-04-10: Milestone 2 CRUD wired to real DB: Places repository injected via `forFeature` + `InjectRepository`, create/find/update/delete now persist in PostgreSQL, and delete returns 204. Fixed metadata/table-column mapping issues by enabling `autoLoadEntities`, mapping entity to `places`, and mapping timestamp fields to `created_at`/`updated_at`. Key learned: entity metadata registration and DB column naming must align with migrations. Next step: start Milestone 3 (nearby radius search with `ST_DWithin`).
